function _magnum_configure() {
    crudini --set $MAGNUM_CONF api host $MGMT_IP
    crudini --set $MAGNUM_CONF certificates cert_manager_type x509keypair
    ...
    crudini --set $MAGNUM_CONF DEFAULT log_file magnum.log
    crudini --set $MAGNUM_CONF DEFAULT log_dir /var/log/magnum
    ...
    crudini --set $MAGNUM_CONF keystone_authtoken auth_uri http://$CTRL_MGMT_IP:5000/v3
    ...
    crudini --set $MAGNUM_CONF trust trustee_domain_admin_password $MAGNUM_DOMAIN_ADMIN_PASS
